...
public class SteeringBehaviours
{
    ...
    static public Vector3 InsideRectangle(SBAgent agent, Transform target, Transform center, float sizeX, float sizeY)
    {
		...
		|| agent.transform.position.y < center.position.y - sizeY || agent.transform.position.y > center.position.y + sizeY)
        {
            steer = Seek(agent, center);
        }
        else
